...
public class KdTree {
	...
	private static enum Dimension{
		...
		RectHV getNextLeftContainer(RectHV previousRectangle, Point2D p){
			switch(this){
			case X:
				return new RectHV(previousRectangle.xmin(), previousRectangle.ymin(), p.x(), previousRectangle.ymax());
			case Y:
				return new RectHV(previousRectangle.xmin(), previousRectangle.ymin(), previousRectangle.xmax(), p.y());
			}
			return null;
		}
		RectHV getNextRightContainer(RectHV previousRectangle, Point2D p){
			switch (this) {
			case X:
				return new RectHV(p.x(), previousRectangle.ymin(), previousRectangle.xmax(), previousRectangle.ymax());
			case Y:
				return new RectHV(previousRectangle.xmin(), p.y(), previousRectangle.xmax(), previousRectangle.ymax());
			}
			return null;
		}
	}
	...
	private KdNode insertInto(KdNode node, Point2D key, Dimension d){
		if(!isEmpty(node)){
			...
			if(cmp <= 0) node.left = insertInto(node.left, key, d.next());
			else if(cmp > 0) node.right = insertInto(node.right, key, d.next());
		} else{
			node.key = key;
			node.left = new KdNode(d.getNextLeftContainer(node.container, key));
			node.right = new KdNode(d.getNextRightContainer(node.container, key));
		}
	...
	private boolean contains(KdNode node, Point2D p, Dimension d){
		...
		if(cmp < 0) return contains(node.left, p, d.next());
		if(cmp > 0) return contains(node.right, p, d.next());
		return true;
	}
	...
	private void accumulateIntersectionWithNode(RectHV rect, KdNode node, Queue<Point2D> q){
		if(!rect.intersects(node.container) || isEmpty(node)) return;
	...
	public static void main(String[] args) {
        ...
        kdtree.draw();
        kdtree.range(new RectHV(0.15, 0.05, 0.25, 0.15));
	}
